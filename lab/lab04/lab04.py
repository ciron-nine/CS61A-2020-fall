...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    cur=make_city('cur',lat,lon)
    a=distance(cur,city_a)
    b=distance(cur,city_b)
    if a<b:
...
def add_chars(w1, w2):
    ...
    """
    "*** YOUR CODE HERE ***" 
    def sub(index,w3=w1,w4=w2):
        if w3[0]==w4[index]:
            if len(w3)==1:
                if(index+1==len(w4)):
                    w4=w4[:len(w4)-1]
                else:
                    w4=w4[:index]+w4[-1*(len(w4)-index-1):]
                return w4
            else:
                w3=w3[1:]
                w4=w4[:index]+w4[-1*(len(w4)-index-1):]
                return sub(index,w3,w4)
        else:
            return sub(index+1,w3,w4)
