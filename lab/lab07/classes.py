...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(length):
            opponent.draw()
        print(opponent.name+' discarded and re-drew '+str(length)+' cards!')
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        added_attack=other_card.attack
        added_defense=other_card.defense
        new_cards=[]
        for i in player.deck.cards:
            new_cards+=[Card(i.name,i.attack+added_attack,i.defense+added_defense)]
        player.deck=Deck(new_cards)
        index_list=[]
        testlength=orig_opponent_deck_length-1
        while testlength>=0:
            if opponent.deck.cards[testlength].attack==added_attack and opponent.deck.cards[testlength].defense==added_defense:
                index_list+=[testlength]
            testlength-=1
        for i in index_list:
