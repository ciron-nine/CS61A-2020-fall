...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [deck[(x%2==1 and len(deck)//2)+x//2] for x in range(len(deck)) ]
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t)==True:
      newbranches=[]
      for i in leaves:
        newbranches=newbranches+[tree(i)]
      t=tree(label(t),newbranches)
    else:
      newbranches=[]
      for i in branches(t):
        newbranches+=[sprout_leaves(i,leaves)]
      t=tree(label(t),newbranches)
    return t
