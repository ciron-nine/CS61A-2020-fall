...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        length=self.min_range
        while flag and length<=self.max_range:
            if cur_place==beehive:
                flag=False
            elif len(cur_place.bees)!=0:
                nearest_bee=rANTdom_else_none(cur_place.bees)
                flag=False
            cur_place=cur_place.entrance
            length+=1
        return nearest_bee# REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        while index>=0:
            if self.place.bees[index].armor-hurt<=0:
                Insect.reduce_armor(self.place.bees[index],hurt)
            else:
                Insect.reduce_armor(self.place.bees[index],hurt)
            index-=1
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.test_one:
            self.is_impostor=False
            QueenAnt.test_one=False
        else:
    ...
    def action(self, gamestate):
        ...
        if  self.is_impostor:
            ...
            while cur_place!=None:
                if( not (cur_place.ant in self.marked_ants)) and cur_place.ant!=None:
                    cur_place.ant.damage=cur_place.ant.damage*2
                    self.marked_ants+=[cur_place.ant]
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        if self.contained_ant!=None:
            return False
        elif  isinstance(other,ContainerAnt):
            return False
        else:
            return True
        # END Problem Optional 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        if isinstance(self.place.ant,ContainerAnt):
            newdict.update({self.place.ant:0})
        if len(self.place.bees)!=0:
            for bee in self.place.bees:
                newdict.update({bee:0})
        while cur_place!=beehive:
            if cur_place.ant !=None:
                if isinstance(cur_place.ant,ContainerAnt):
                    newdict.update({cur_place.ant:length})
                    newdict.update({cur_place.ant.contained_ant:length})
                else:
                    newdict.update({cur_place.ant:length})
            if len(cur_place.bees)!=0:
                for bee in cur_place.bees:
                    newdict.update({bee:length})
            length+=1
            cur_place=cur_place.entrance
        return newdict
        # END Problem Optional 5
    ...
    def calculate_damage(self, distance):
        ...
        if self.damage-0.05*self.insects_shot-0.2*distance<0:
            return 0
        return self.damage-0.05*self.insects_shot-distance*0.2
        # END Problem Optional 5
