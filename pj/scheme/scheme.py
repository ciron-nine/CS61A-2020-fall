...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if self.bindings.get(symbol) is not None:
            ...
            raise SchemeError
        elif self.parent.lookup(symbol) is not None:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        #if not isinstance(expressions.rest.first,Pair):
        #    rest=expressions.rest.first
        #    if env.bindings.get(rest) is not None:
        #         rest=env.bindings[rest]
        #    env.define(target,rest)
        #else:
        #    def replace(pair,env):
        #        if pair ==nil:
        #            return nil
        #        if isinstance(pair.first,Pair):
        #            return Pair(replace(pair.first,env),replace(pair.rest,env))
        #        else:
        #            if env.bindings.get(pair.first)is not None and (not isinstance(env.bindings.get(pair.first),Procedure)):
        #                return Pair(env.bindings[pair.first],replace(pair.rest,env))
        #            else:
        #               return Pair(pair.first,replace(pair.rest,env))
        #    rest=replace(expressions.rest.first,env)
            #rest=expressions.rest.first.map(lambda x:env.bindings[x] if env.bindings.get(x)is not None  else x)
      ...
      #  return expressions.first
        ...
        env.bindings.update({target.first:do_lambda_form(expression,env)})
        return target.first
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings!=nil:
        ...
        values=Pair(eval_all(bindings.rest,env),values)
